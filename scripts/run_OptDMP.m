...
addpath('OptDMP/') % Opt-DMP code
addpath('utils/') % Support Scripts

% Define output paths
demos_path = '../Data/demos/';
output_trajectories_path = '../Data/trajectories';
output_plots_path = '../Data/plots/';

% Check if the output trajectories path exist
if ~exist(output_trajectories_path, 'dir')
    mkdir(output_trajectories_path)
end
if ~exist(output_plots_path, 'dir')
    mkdir(output_plots_path)
end

% Add demonstration trajectory path
addpath(demos_path)
...
writematrix(unconstrained_DMP.pos',fullfile(output_trajectories_path,strcat(bag,'_UC_Opt_joint_',filename)))
writematrix(unconstrained_DMP.vel',fullfile(output_trajectories_path,strcat(bag,'_UC_Opt_joint_vel_',filename)))
...
res.pos(5,:) = -res.pos(5,:);
res.pos(7,:) = -res.pos(7,:);
writematrix(res.pos',fullfile(output_trajectories_path,strcat(bag,'_Opt_DMP_joint_',filename)))

res.vel(1,:) = -res.vel(1,:);
res.vel(3,:) = -res.vel(3,:);
res.vel(5,:) = -res.vel(5,:);
res.vel(7,:) = -res.vel(7,:);
writematrix(res.vel',fullfile(output_trajectories_path,strcat(bag,'_Opt_DMP_joint_vel_',filename)))

res.acc(1,:) = -res.acc(1,:);
res.acc(3,:) = -res.acc(3,:);
res.acc(5,:) = -res.acc(5,:);
res.acc(7,:) = -res.acc(7,:);
writematrix(res.acc',fullfile(output_trajectories_path,strcat(bag,'_Opt_DMP_joint_acc_',filename)))
...
poseDMP = ForwardKinematics(res.pos');
writematrix(poseDMP,fullfile(output_trajectories_path,strcat(bag,'_Opt_DMP_pose_',filename)))
