...
% Avoid showing all the figures
set(0,'DefaultFigureVisible','off')

function plotJointTime(index, res, p_min, p_max, v_max, a_max, unconstrained_DMP, output_plots_path)
    figure('Name',strcat('joint',int2str(index)), 'Position', get(0, 'Screensize'));
    ...
    legend([unconstrained_dmp_plot, constrained_dmp_plot])
    hold off
    saveas(gcf, strcat(output_plots_path,'joint_',int2str(index),'.png'))
end

%plots for all joints
for joint = 1:7
    plotJointTime(joint, res, p_min, p_max, v_max, a_max, unconstrained_DMP, output_plots_path);
end


%%%
figure('Name','Cartesian Pose', 'Position', get(0, 'Screensize'));
subplot(2,1,1)
hold on
...
legend
hold off
saveas(gcf, strcat(output_plots_path,'cartesian_pose.png'))


%% Joint Velocity plots
figure('Name', 'Joint velocities', 'Position', get(0, 'Screensize'));
...
saveas(gcf, strcat(output_plots_path,'joint_velocities.png'))

%% Joint Acceleration plots
figure('Name', 'Joint accelerations', 'Position', get(0, 'Screensize'));
...
saveas(gcf, strcat(output_plots_path,'joint_accelerations.png'))
